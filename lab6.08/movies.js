...
const calculateAverageScores = () => {
  const scores = movies.map(movie => movie.score); const averageScore = scores.reduce((total, score) => total + score, 0) / scores.length;
...
// 1. Create an object to store the sum and count of scores for each year
const yearScoreSum = {};
const yearScoreCount = {};
// 2. Calculate the sum and count of scores for each year
movies.forEach(movie => {
  const year = movie.year;
  const score = movie.score;

  if (!yearScoreSum[year]) {
    yearScoreSum[year] = 0;
    yearScoreCount[year] = 0;
  }

  yearScoreSum[year] += score;
  yearScoreCount[year]++;
});

// 3. Calculate the average score for each year
const yearScoreAverages = {};
for (const year in yearScoreSum) {
  yearScoreAverages[year] = yearScoreSum[year] / yearScoreCount[year];
}

// 4. Find the year with the highest average score
let bestYear = null;
let bestAverage = -1;

for (const year in yearScoreAverages) {
  if (yearScoreAverages[year] > bestAverage) {
    bestYear = year;
    bestAverage = yearScoreAverages[year];
  }
}

console.log(`------------------------------------------------------The year with the highest average score is:------------------------------------------------------\n ${bestYear} with an average score of ${bestAverage.toFixed(2)}`);
